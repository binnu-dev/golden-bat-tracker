...
const GoldenBatValue = () => {
  ...
  // 현재 금시세 (mock - API에서 가져온다고 가정)
  const CURRENT_GOLD_PRICE_PER_G = 82000; // KRW/g

  // 초기 투자금 및 현재 가치 계산
  const INITIAL_VALUE = 2700000000; // 27억원
  const CURRENT_VALUE = GOLD_WEIGHT_G * CURRENT_GOLD_PRICE_PER_G;
  const valueIncrease = CURRENT_VALUE / INITIAL_VALUE;
  const cagr = (Math.pow(valueIncrease, 1/YEARS_PASSED) - 1) * 100;
          ...
          <div className="relative w-full max-w-2xl h-64">
            <svg viewBox="0 0 800 200" className="w-full">
              ...
              <g transform="translate(600, 100)">
                ...
                <text x="0" y="0" textAnchor="middle" className="text-2xl font-bold" fill="#1f2937">
                  {(CURRENT_VALUE / 100000000).toFixed(0)}억
                </text>
                <text x="0" y="25" textAnchor="middle" className="text-sm" fill="#4b5563">
                  {CURRENT_YEAR}년
                </text>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
            ...
            <div className="p-4 bg-amber-50 rounded-lg">
              ...
              <p className="text-lg font-bold text-amber-900">
                {CURRENT_GOLD_PRICE_PER_G.toLocaleString()}원/g
              </p>
            </div>
            ...
            <div className="p-4 bg-emerald-50 rounded-lg">
              ...
              <p className="text-lg font-bold text-emerald-900">
                {(CURRENT_VALUE / 100000000).toFixed(1)}억원
              </p>
            </div>
          </div>
        </CardContent>
